    ...
    <div class="container">
        ...
        <v-container  class="mt-6" v-for="(post, index) in posts" :key="index">
            <v-row justify="space-around">
                <v-card elevation="10" width="700">
                    <v-img v-if="post.imageUrl" height="400px" :src="post.imageUrl">
                        <v-app-bar flat color="rgba(0, 0, 0, 0)">
                            <v-toolbar-title
                                class="text-subtitle-2 white--text pl-0"
                            >Posté le {{ dateParser(post.createdAt) }}</v-toolbar-title>
                            <v-spacer></v-spacer>
                            <v-col
                                cols="2"
                        ...
                        <v-card-title class="white--text mt-8">
                            ...
                            <p
                                class="ml-3"
                                @click="profil(post.UserId)"
                            >{{ post.User.last_name }} {{ post.User.first_name }}</p>
                        </v-card-title>
                    </v-img>
                    <v-img v-else height="200px" src="">
                        <v-app-bar flat color="rgba(0, 0, 0, 0)">
                            <v-toolbar-title
                                class="text-subtitle-2 white--text pl-0"
                            >Posté le : {{ dateParser(post.createdAt) }}</v-toolbar-title>
                    ...
                    <v-card-text>
                        <div class="font-weight-bold ml-8 mb-2">{{ post.User.first_name }} a écrit :</div>
                        <div class="offset-md-1 mx-auto">
                            <v-alert shaped dark text color="info">{{ post.message }}</v-alert>
                        </div>
                        <div>
                            <v-col width="900" class="d-flex justify-end">
                                <v-btn @click="commentPost(post.id)" color="primary">
                                    Écrire un commentaire
                                    <v-icon right dark>mdi-border-color</v-icon>
                                </v-btn>
                            </v-col>
                        </div>
                        ...
                        <v-expand-transition>
                            <div v-show="show.includes(index)">
                                ...
                                >
                                    ...
                                    <v-icon
                                        v-if="comment.UserId == UserId || userAdmin === true"
                                        class="cross"
                                        color="error"
